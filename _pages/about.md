...
At forecast initialization, TP fields are illustrated in Panel (a). Panel (b) showcases the TP forecast generated by FourCastNet over the entire globe at a 0.25° lat-long resolution, juxtaposed with the corresponding truth. Inset 1 zooms into a local region along the western coast of the United States, displaying the model's ability to predict localized regions of high precipitation, such as those caused by atmospheric rivers. Inset 2 focuses on a region near the coast of the U.K., highlighting an extreme precipitation event due to an extra-tropical cyclone predicted accurately by FourCastNet. The precipitation is diagnosed from FourCastNet-predicted prognostic variables, with the initial condition timestamped at 00:00 UTC on April 4, 2018. The high-resolution FourCastNet model excels in capturing small-scale features crucial for precipitation forecasting.

![Example 2](/images/Example2.png)

In a forecast initialized at 00:00 UTC on April 4, 2018, Total Column Water Vapor (TCWV) is visualized in Panels (a)-(c). Panel (a) illustrates the TCWV field in the initial condition used for initializing the FourCastNet model. Panels (b) and (c) display the forecasts of the TCWV field generated by the FourCastNet model (top panels) at lead times of 36 and 72 hours, respectively, alongside the corresponding true TCWV fields at those times. The forecast depicts the development of an atmospheric river culminating in landfall on the northern California coastline.
